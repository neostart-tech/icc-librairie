<template>
  <section class="relative w-full h-[400px] md:h-[500px] lg:h-[600px] overflow-hidden group">
      ...
      <div class="text-left text-white max-w-2xl transform transition-all duration-700 translate-y-0 opacity-100">
        <h1 class="animate-title text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold mb-4 leading-tight tracking-wide">
          ...
          <span class="relative inline-block mt-1 sm:mt-2 group/title">
            ...
            <svg class="absolute w-[105%] h-[12px] md:h-[18px] -bottom-1 -left-1 text-[#6a0d5f] z-0 drop-shadow-md max-w-full" viewBox="0 0 100 10" preserveAspectRatio="none">
              <path pathLength="100" class="path-underline" d="M 0 8 Q 50 0 100 8" stroke="currentColor" stroke-width="4" fill="none" stroke-linecap="round"/>
            </svg>
          </span>
        </h1>
        ...
        <button
          ...
          class="animate-button group/btn relative overflow-hidden px-8 py-4 bg-[#6a0d5f] rounded-full text-base md:text-lg font-bold shadow-xl hover:shadow-[#6a0d5f]/50 transition-all duration-300 transform hover:-translate-y-1"
        >
    ...
    <button @click="prevSlide" class="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/20 hover:bg-black/50 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 backdrop-blur-sm">
      <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
      </svg>
    </button>
    <button @click="nextSlide" class="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/20 hover:bg-black/50 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 backdrop-blur-sm">
      <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="absolute bottom-8 left-0 right-0 z-20 flex justify-center gap-3">
      ...
      >
        ...
        <span 
          ...
          class="absolute inset-y-0 left-0 bg-white animate-[progress_5s_linear]"
        ></span>
      </button>
    </div>
  </section>
...
// On liste les images du dossier public/images/banniere/
const images = [
  '/images/banniere/7_ARMES_DE_DIEU_1920x1080_6021f1d3-7ec0-408e-a113-50549accf101.webp',
  '/images/banniere/Banniere_2.webp',
  '/images/banniere/Banniere_esther_f24228d2-40bf-451e-8466-29f929a6a4ed.webp',
  '/images/banniere/Jade_version_desktop_1.webp',
  '/images/banniere/MAXIMISER_VOTRE_TEMPS_1920x1080_18697059-f98b-4db2-8817-090a756a60f9.webp'
];
...
const resetInterval = () => {
  ...
  // Interval de 5 secondes correspondant à l'animation
...
const scrollToSection = (id, offset = 100) => {
  ...
  if (element) {
    ...
    // La hauteur du header est maintenant plus grande, donc on peut mettre un plus grand offset (ex: 120-130)
    const headerHeightOffset = 130; 
    const offsetPosition = elementPosition - headerHeightOffset;
...
@keyframes textPulse {
  0%, 100% { text-shadow: 0px 4px 15px rgba(0,0,0,0.5), 0px 0px 5px rgba(255,255,255,0.1); }
  50% { text-shadow: 0px 4px 20px rgba(0,0,0,0.6), 0px 0px 15px rgba(255,255,255,0.5); }
}
...
@keyframes floatWord {
  ...
  50% { transform: translateY(-4px); color: #fdf2f8; } /* pink-50 */
}
