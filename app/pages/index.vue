  ...
  <div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-6">
      ...
      <div class="relative" :class="{ 'z-40': isFilterOpen, 'z-30': !isFilterOpen }">
        <FiltersDropdown :categories="categorieStore.categories" :min-price="0" :max-price="20000"
          @update:filters="updateFilters" @open-change="(val) => isFilterOpen = val" />
      </div>

      <!-- CONTENU - Z-INDEX BAS -->

      <!-- NOUVEL EMPLACEMENT DE LA BARRE DE RECHERCHE -->
      <div class="mb-6 w-full max-w-lg mx-auto z-10 relative mt-2">
        <div
          class="group/search flex items-center bg-white border border-gray-200 rounded-full px-4 py-2.5 w-full shadow-sm transition-all duration-300 hover:shadow-md focus-within:border-[#6a0d5f] focus-within:ring-1 focus-within:ring-[#6a0d5f]/20 cursor-text">
          <input v-model="search" type="text" placeholder="Rechercher un livre, un auteur..."
            class="w-full text-sm text-gray-800 placeholder-gray-400 outline-none bg-transparent transition-colors" />
          <span
            class="ml-2 text-gray-400 group-focus-within/search:text-[#6a0d5f] transition-all duration-300">
      ...
      <section id="livres-list" class="flex-1 mt-2 relative z-0">
        <!-- LIVRES -->
        <div v-if="paginatedBooks.length > 0"
          ...
          <div v-for="book in paginatedBooks" :key="book.id"
            ...
            <div
              class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-400 rounded-xl z-20 flex flex-col items-center justify-center p-4">
              ...
              <!-- Voir détails -->
              <span
                class="text-white/80 text-xs flex items-center gap-1 hover:text-white transition-colors cursor-pointer relative z-30"
                @click.stop.prevent="$router.push(`/livres/${book.id}`)">
              ...
              <span v-if="book.stockAvailable > 0 && book.stockAvailable <= 3"
                class="absolute bottom-3 left-0 right-0 text-center text-white/90 text-[10px] font-medium">
            ...
            <div class="p-3 flex flex-col flex-grow bg-white rounded-b-xl relative z-0">
              ...
              <div class="flex items-center justify-between mt-auto">
                <div class="flex flex-col">
                  ...
                  <span v-if="book.stockAvailable > 3"
                    class="text-[9px] text-green-600 mt-0.5 flex items-center gap-0.5">
            ...
            <div v-if="cartStore.getQuantity(book.id) >= 1 && book.stockAvailable > 0"
          ...
          <nav class="flex items-center gap-1 bg-white rounded-lg shadow-sm p-1">
            ...
            <template v-for="(page, index) in displayedPages" :key="index">
              ...
              <button v-else @click="changePage(page)"
                class="min-w-[2.2rem] px-2.5 py-1.5 rounded-md text-xs font-medium transition-all" :class="page === currentPage
