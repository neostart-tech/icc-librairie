    ...
    <div v-reveal class="relative z-10 w-full max-w-4xl bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row min-h-[540px]">
      ...
      <div class="hidden md:flex w-[45%] bg-[#6a0d5f] flex-col justify-between p-12 relative overflow-hidden"
        ...
        <div class="relative z-10 space-y-6">
          <div>
            <p class="text-white/60 text-xs font-bold uppercase tracking-widest mb-2">Authentification via</p>
            <h2 class="text-3xl font-bold text-white leading-snug">
              Compte<br/><span class="text-white/80">ICC-Covoiturage</span>
            </h2>
            <p class="mt-3 text-white/60 text-sm leading-relaxed">
              Utilisez vos identifiants ICC uniques pour accéder à tout l'écosystème ICC, y compris la librairie.
            </p>
          </div>
      ...
      <div class="flex-1 flex flex-col justify-center p-8 md:p-12">
        ...
        <div class="mb-8">
          <h1 class="text-2xl font-bold text-gray-900">Connexion ICC</h1>
          <p class="text-gray-400 text-sm mt-1">Entrez vos identifiants ICC-Covoiturage</p>
        </div>
...
const features = [
  ...
  { icon: 'i-lucide-book-open', label: 'Accès à la librairie ICC' },
  { icon: 'i-lucide-shield-check', label: 'Connexion sécurisée unifiée' },
];
...
const handleLogin = async () => {
  try {
    ...
    Swal.fire({
      ...
      customClass: { popup: 'rounded-xl border-l-4 border-purple-500 shadow-xl' }
    });
  } catch (e: any) {
    Swal.fire({
      title: 'Erreur ICC',
