  ...
  <div class="min-h-screen bg-gray-50 pb-20">
    ...
    <section class="relative bg-[#6a0d5f] pt-24 pb-36 overflow-hidden">
       ...
       <div class="max-w-4xl mx-auto px-6 relative z-10 text-center">
          <NuxtLink v-reveal to="/dashboard/commandes" class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/10 text-white/80 text-[10px] font-bold uppercase tracking-wide mb-8 hover:bg-white/20 transition-all">
          ...
          <div v-reveal class="flex flex-wrap items-center justify-center gap-4 text-white/60 text-xs font-bold uppercase tracking-widest mt-6">
    ...
    <main class="max-w-4xl mx-auto px-6 -mt-16 relative z-20">
      <div v-if="commande" class="space-y-8">
        ...
        <div v-reveal class="bg-white rounded-xl shadow-2xl p-6 md:p-8 border border-white">
           ...
           <div class="space-y-6">
              <div v-for="(detail, idx) in commande.detailcommandes" :key="detail.id" class="flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100 group hover:border-[#6a0d5f]/20 transition-all cursor-default">
                <NuxtLink :to="`/livres/${detail.livre?.id}`" class="flex items-center gap-4 w-full md:w-auto hover:opacity-80 transition-opacity">
                   <div class="w-14 h-20 rounded-lg bg-gray-200 overflow-hidden shadow-md border-2 border-white group-hover:scale-110 transition-transform duration-500">
                   ...
                   <div class="space-y-1">
                      <h3 class="text-sm font-bold text-gray-900 tracking-wide leading-tight group-hover:text-[#6a0d5f] transition-colors">{{ detail.livre?.titre }}</h3>
                ...
                <div class="w-full md:w-auto text-right md:text-right border-t md:border-t-0 border-gray-100 pt-4 md:pt-0">
                   <span class="text-base font-bold text-[#6a0d5f] tracking-wide">{{ (detail.prix_unitaire * detail.quantite).toLocaleString() }} FCFA</span>
                </div>
              </div>
           </div>
           ...
           <div class="mt-6 pt-6 border-t border-gray-100 space-y-3">
              ...
              <div class="pt-4 flex justify-between items-end">
                <div class="space-y-1">
                   <p class="text-[10px] font-bold text-orange-500 uppercase tracking-widest">Montant Total Réglé</p>
                   <p class="text-2xl font-bold text-[#6a0d5f] tracking-wide">{{ commande.prix_total.toLocaleString() }} <span class="text-sm font-bold ml-1">FCFA</span></p>
                </div>
                <div class="hidden sm:block px-4 py-2 bg-gray-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl">
                 ...
                 <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-500">
                     ...
                     <div>
                        ...
                        <p class="font-bold text-gray-900 tracking-wide">{{ commande.user?.prenom || userStore.user?.prenom }} {{ commande.user?.nom || userStore.user?.nom }}</p>
                     </div>
                  </div>
                  <div class="flex items-center gap-4">
                     ...
                     <div>
                        ...
                        <p class="font-bold text-gray-900 tracking-wide">{{ commande.user?.email || userStore.user?.email }}</p>
                     </div>
                  </div>
              </div>
           </div>
