  ...
  <div class="min-h-screen bg-gray-50 pb-20">
    ...
    <section class="relative bg-[#6a0d5f] pt-32 pb-48 overflow-hidden">
      <!-- Decor -->
       ...
       <div class="max-w-7xl mx-auto px-6 relative z-10 text-center md:text-left">
          ...
          <h1 v-reveal class="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter leading-none mb-4">
    ...
    <div class="max-w-7xl mx-auto px-6 -mt-24 relative z-20">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-reveal class="reveal-delay-100 bg-white shadow-2xl shadow-purple-900/5 rounded-[2.5rem] p-8 border border-white flex items-center justify-between group hover:scale-[1.02] transition-all duration-500">
           <div class="space-y-1">
             <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Commandes</p>
             <p class="text-4xl font-black text-[#6a0d5f] italic tracking-tighter">{{ totalProcessedOrders }}</p>
           </div>
           <div class="w-14 h-14 rounded-2xl bg-gray-50 flex items-center justify-center text-[#6a0d5f] group-hover:bg-[#6a0d5f] group-hover:text-white transition-all duration-300">
        ...
        <div v-reveal class="reveal-delay-200 bg-white shadow-2xl shadow-purple-900/5 rounded-[2.5rem] p-8 border border-white flex items-center justify-between group hover:scale-[1.02] transition-all duration-500">
           <div class="space-y-1">
             <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Investissement Total</p>
             <p class="text-4xl font-black text-orange-500 italic tracking-tighter">{{ totalSpent }} <span class="text-base font-bold ml-1">FCFA</span></p>
           </div>
           <div class="w-14 h-14 rounded-2xl bg-gray-50 flex items-center justify-center text-orange-500 group-hover:bg-orange-500 group-hover:text-white transition-all duration-300">
        ...
        <div v-reveal class="reveal-delay-300 bg-white shadow-2xl shadow-purple-900/5 rounded-[3rem] p-8 border border-white flex items-center justify-between group hover:scale-[1.02] transition-all duration-500">
           <div class="space-y-1">
             <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Panier Moyen</p>
             <p class="text-4xl font-black text-blue-500 italic tracking-tighter">{{ averageOrder }} <span class="text-base font-bold ml-1">FCFA</span></p>
           </div>
           <div class="w-14 h-14 rounded-2xl bg-gray-50 flex items-center justify-center text-blue-500 group-hover:bg-blue-500 group-hover:text-white transition-all duration-300">
             <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
           </div>
        </div>
      </div>
    </div>
    ...
    <main class="max-w-7xl mx-auto px-6 mt-16">
      <div v-reveal class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        <div class="space-y-1">
          <h2 class="text-2xl font-black text-gray-900 uppercase italic tracking-tighter">Historique des transactions</h2>
      ...
      <div v-if="paginatedOrders.length > 0" class="space-y-4">
        <div 
          ...
          :key="order.reference"
          v-reveal
          :class="`reveal-delay-${(idx % 5 + 1) * 100}`"
          class="bg-white rounded-[2rem] p-6 md:p-8 shadow-xl shadow-purple-900/5 border border-white flex flex-col md:flex-row items-center justify-between gap-6 hover:shadow-2xl hover:border-[#6a0d5f]/10 transition-all duration-500 group"
        >
          <!-- Order Info -->
          <div class="flex items-center gap-6 w-full md:w-auto">
             <div class="w-16 h-16 rounded-2xl bg-[#6a0d5f]/5 flex items-center justify-center text-[#6a0d5f] font-black text-lg group-hover:bg-[#6a0d5f] group-hover:text-white transition-colors duration-500">
               {{ (currentPage - 1) * itemsPerPage + idx + 1 }}
             </div>
             <div class="space-y-1">
               <div class="flex items-center gap-3">
                 <span class="text-xl font-black text-gray-900 tracking-tighter">#{{ order.reference }}</span>
                   <span 
                    class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm bg-green-100 text-green-600"
                  >
                    Payée
                  </span>
               </div>
          ...
          <div class="w-full md:w-auto flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-2">
             <div class="text-2xl font-black text-[#6a0d5f] italic tracking-tighter">{{ order.montant }}</div>
             <div class="flex items-center gap-2 px-3 py-1 bg-gray-50 rounded-lg text-gray-400">
                <span class="text-[8px] font-black uppercase tracking-widest items-center flex gap-1">
          ...
          <button 
            ...
            class="w-full md:w-auto bg-gray-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-[#6a0d5f] hover:scale-105 active:scale-95 transition-all duration-300 shadow-xl shadow-black/10"
          >
            Détails Commande
          </button>
        </div>
      ...
      <!-- Empty State -->
      <div v-else v-reveal class="bg-white rounded-[3rem] p-20 text-center border-4 border-dashed border-gray-100 mt-10">
        ...
        <h3 class="text-3xl font-black text-gray-900 italic tracking-tighter mb-4 uppercase">Aucune commande</h3>
    ...
    <Transition name="modal">
      ...
      >
        ...
        <div v-reveal class="relative w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          <!-- Header Modal -->
          <div class="p-8 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
            <div class="space-y-1">
              <h3 class="text-2xl font-black text-gray-900 uppercase italic tracking-tighter">Facture #{{ selectedCommande?.reference }}</h3>
          ...
          <div class="flex-1 overflow-y-auto p-8 space-y-10">
              ...
              <div class="space-y-4">
                <div v-for="d in selectedCommande?.detailcommandes" :key="d.id" class="p-5 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between gap-4 group hover:border-[#6a0d5f]/20 transition-all">
                  <NuxtLink :to="`/livres/${d.livre?.id}`" class="flex items-center gap-4 hover:opacity-80 transition-opacity flex-1">
                    ...
                    <div>
                      <h5 class="font-black text-gray-900 text-sm italic tracking-tight group-hover:text-[#6a0d5f] transition-colors">{{ d.livre?.titre }}</h5>
            ...
            <!-- Summary -->
            <div class="bg-gray-900 rounded-[2rem] p-8 text-white space-y-6 shadow-2xl">
               ...
               <div class="flex justify-between items-end">
                 <div class="space-y-1">
                   ...
                   <p class="text-3xl font-black italic tracking-tighter">{{ selectedCommande?.prix_total.toLocaleString() }} <span class="text-sm border-l border-white/20 ml-2 pl-2">FCFA</span></p>
                 </div>
                 <div class="bg-white/10 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-white/10">Facture Payée</div>
               </div>
            </div>
            ...
            <div class="space-y-4">
               ...
               <div class="grid grid-cols-2 gap-4">
                 <div class="p-6 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col gap-2">
                 ...
                 <div class="p-6 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col gap-2 overflow-hidden">
          ...
          <!-- Footer Modal -->
          <div class="p-8 bg-white border-t border-gray-100 flex justify-center">
            <button @click="showDetailsModal = false" class="w-full bg-gray-900 text-white py-5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-[0.3em] hover:bg-[#6a0d5f] transition-all duration-300">
